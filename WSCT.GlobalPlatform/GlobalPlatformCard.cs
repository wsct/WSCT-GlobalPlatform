...
{
    private const int loadChunkSize = 0xE0;
    ...
    /// <summary>
    /// Gets the `Card Data` data object (tag '66').
    /// </summary>
    public CardData CardData
    {
        get
        {
            GlobalPlatformException.ThrowIfNull(_cardData);
    ...
    /// <summary>
    /// Gets the Secure Channel Data in use.<br/>
    /// Warning: This property is for information only and should not be used to modify the secure channel.
    /// </summary>
    public SecureChannelData SecureChannelData
    {
        get
        {
    ...
    /// </summary>
    /// <returns>Whether the card was authenticated successfully.</returns>
    ...
    {
        using var capFile = new CapFile(rawCapFileStream);
        ...
        foreach (var chunk in loadData.Chunk(loadChunkSize))
        {
    ...
    {
        using var file = File.OpenRead(pathToCapFile);

        return ProcessLoad(file);
    }
